...
### 4.2. Service E-mail (`ArchivEx/accounts/services.py`)
...
- Utilise le gabarit d'e-mail `ArchivEx/templates/emails/password_reset_email.html` arborant la charte graphique ArchivEx (logo, bannière indigo/bleue, carte récapitulative des identifiants, bouton d'action direct vers `/connexion/`, consignes de sécurité).
...
### 4.3. Modèle de Données (`ArchivEx/support/models.py`)
- La catégorie `recuperation_mot_de_passe` a été ajoutée aux choix possibles de `SupportRequest.CATEGORY_CHOICES`.
- La migration associée est : `ArchivEx/support/migrations/0003_alter_supportrequest_category.py`.

---

## 🧪 5. Tests Automatisés

Une suite complète de 5 tests unitaires et fonctionnels est disponible dans `ArchivEx/support/tests.py` :
